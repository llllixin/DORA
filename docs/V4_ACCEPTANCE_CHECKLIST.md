...
> 对应：docs/历史版本路线.md §3B（V4 规划已归档）。V4-T1..T5（迭代 22–26）归档后填写；自动项给出命令，人工项给出核对标准。
> 快照说明：本文为 V4 sign-off 验收记录（A/B/C 已确认，2026-09-07），**此后不再更新**；后续 backlog 见 `docs/持续优化路线.md`（文档职责分层见 `docs/文档地图.md`）。
...
## C. 功能人工核对（需在运行界面确认）
- [x] 打开 http://localhost:5173/watch：输入"帮我关注华东销售额，如果连续三天下降就提醒我" → 让 Dora 理解 → 出现解析回显（业务目标/指标口径/触发条件/频率）→ 开始持续关注 → 列表出现该委托（观察中）
- [x] 点击「检查」手动评估；「暂停」后状态=已暂停且再上传数据不产生新事件；「恢复」可继续；「删除」移出列表
- [x] 上传跌破华东 -3% 的 CSV（或等待调度）后：委托 Tag 变「已升级」(红)，Pulse 持续关注计数随之变化
- [x] Pulse「持续关注」卡片与计数来自真实委托（不再是静态 6 条）；后端离线时才显示本地演示数据

## D. 收尾核对
- [x] dev-log 迭代 22–26、路线图 §3B V4-T1..T5 与版本状态、README 归档索引（17 个 change）、business-watch spec（4 需求 + 来源表）已同步
- [x] 大版本切换记录：sign-off 后把路线图版本速览 V4 标为完成（记录迭代 26）

## Sign-off
| 审查人 | 日期 | 结论（通过/待改） | 备注 |
|---|---|---|---|
| tomara（用户确认） | 2026-09-07 | 通过（V4 标为已完成，迭代 22–26） | C 段界面核对已确认（Watch 委托创建/检查/暂停恢复/删除/升级 Tag/真实 Pulse）；A/B 自动证据见上 |
