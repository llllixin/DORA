...
> 对应：docs/版本路线图.md §3C。V5-T1..T5（迭代 28–33）归档后填写；自动项给命令，人工项给核对标准。
...
## C. 功能人工核对（需在运行界面确认）
- [ ] 洞察详情（problem/opportunity）点「加入行动回路」→ 跳到 Action 页出现该档案（已 seed 的返回既有）；对 change 仍是「加入持续关注」
- [ ] Action 页：列表来自后端；任选档案 → 步骤「开始/完成/受阻」可点击并回填备注/结果（当前步骤备注框）
- [ ] 把所有步骤推进到完成 → 状态=待验证 → 填验证说明点「归档 · 问题解决」→ 显示已归档（含验证记录）；或点「继续观察」→ 状态=执行中
- [ ] 打开 e2 类新问题：上传跌破数据后在引擎判定集内 → 加入行动回路可建档成功（400 情况会显示原因而非伪造成功）
- [ ] 关掉后端再进 Action 页 → 显示"离线演示"兜底（不报错）

## D. 收尾核对
- [ ] dev-log 迭代 28–33、路线图 §3C V5-T1..T5 与版本状态、README 归档索引（25 个 change）、business-action spec（4 需求 + 来源表）已同步
- [ ] 大版本切换记录：sign-off 后把路线图版本速览 V5 标为完成
