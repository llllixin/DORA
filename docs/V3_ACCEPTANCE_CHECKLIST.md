...
## C. 功能人工核对（需在运行界面确认）
- [ ] 洞察详情显示来源徽标（模板解释 / AI 解释）
- [ ] "↻ 重新解释"可用；template 模式 toast「✓ 解释已重新生成（模板模式）」
- [ ] 配置 `DORA_REASONING_PROVIDER=llm` 且无 key 时：刷新返回 fallback、页面保持模板徽标与数据
- [ ] （可选）配置真实 LLM key 后：徽标变「AI 解释」，数值/触发不变

## D. 收尾核对
- [ ] dev-log 迭代、路线图 V3-T5 与版本状态、D 系列记录已同步
- [ ] spec 导航（openspec/README、各能力 spec 需求来源表）已含 V3 新增条目
- [ ] 大版本切换记录：V3 验收通过后把路线图版本速览标为 V3 已完成
